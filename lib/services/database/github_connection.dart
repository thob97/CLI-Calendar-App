...
class Database implements DatabaseStrategy {
  ...
  Future<bool> autoSetup() async {
    ...
    ///const variables
    // repo
    final String now = DateFormat.yMd()
        .add_Hms()
        .format(DateTime.now())
        .replaceAll(RegExp(r'\D'), '-');
    final String repoName = 'Calendar-$now';
    ...
    //config
    const String dbConfigPath = 'config.json';
    ...
    onError = !(await uploadIssue(todo: showCaseIssue) != null);
    if (onError) {
      return false;
    }
  ...
  Future<int?> uploadIssue({required Todo todo}) async {
    ...
    for (final TodoFile todoFile in todo.files) {
      ...
      final String githubFilePath =
          '${_config!.issueFileDirPath}/issue_$issueNumber/$filename';
      final String? url = await uploadFile(
        filePath: githubFilePath,
        uploadFile: todoFile.content,
        commitMsg:
            _config!.customCommitMsg ?? 'issue $issueNumber: add $filename ',
      );
